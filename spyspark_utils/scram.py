...
import spyspark_utils.scram_utils as scram_u
...
def hello(url, username: str) -> dict:
...
def first_message(url, username: str) -> dict:
...
def final_message(url, username: str = None, salted_password: str = None) -> dict:
    ...
    auth_dict = first_message(url, username)
    if auth_dict['method'] != "scram":
        raise AuthException("Server requested an authentication method" +
                            "other than SCRAM")
    ...
    if not hmac.compare_digest(scram_u.b64_decode(s_msg_values['v'].encode('utf-8')), 
                               s_signature):
